...
type Props = { days: string[]; today: string; storiesRead: number; reflections: number; kindDeeds: number; treasures: number };
...
function Flower({ blooming, fresh, variant }: { blooming: boolean; fresh: boolean; variant: number }) {
  return <span className={`${styles.flower} ${styles[`variant${variant}`]} ${blooming ? styles.blooming : ""} ${fresh ? styles.fresh : ""}`} aria-hidden="true"><i/><b/><em/><strong/><small/></span>;
}

export default function JourneyGarden({ days, today, storiesRead, reflections, kindDeeds, treasures }: Props) {
  ...
  const newFlowers = Math.max(0, flowerCount - lastSeenFlowers);
  useEffect(() => {
  ...
  return <section className={styles.garden} aria-labelledby="journey-garden-title">
    ...
    <div className={styles.scene}>
      ...
      {newFlowers > 0 && <span className={styles.bloomNotice}>New bloom</span>}
    </div>
